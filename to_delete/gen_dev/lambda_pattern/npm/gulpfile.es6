...
const jspm = eval('require')(
    ...
    const replace_shared = (code) => {
        ...
        for (var imp of imports) {
            const files = module_glob.sync(imp + '.es6', {
    ...
    const add_regex_includes = (ret, map, imports_code, assign_code, path) => {
        ...
        if (imports.length > 0) {
            ...
            for (var i in imports) {
                const files = module_glob.sync(imports[i], {
                    cwd: __dirname + '/../' + path,
                    root: __dirname + '/../' + path});
    ...
    gulp.task('build_updatables', () => {
        return gulp.src('lambda_pattern/updatables/_.js')
            .pipe(webpack({
    ...
    gulp.task('build_lambda_pattern_tool', () => {
        ...
        return gulp.src('lambda_pattern/_.es6')
            .pipe(webpack({
                externals: [node_externals()],
    ...
    gulp.task('copy_skel_to_parent', () => {
        return gulp.src(['lambda_pattern/skeleton_data/**/*',
                'lambda_pattern/skeleton_data/**/.*'])
    ...
    gulp.task('copy_yaml_to_release', () => {
        return gulp.src([
                'lambda_pattern/shared/default_package_values.yaml',
                'lambda_pattern/shared/package_template.json'])
            .pipe(gulp.dest('../../release/lambda_pattern/shared'));});

    gulp.task('copy_ignore_to_release', () => {
        return gulp.src('lambda_pattern/.gitignore')
            .pipe(gulp.dest('../../release'));});
