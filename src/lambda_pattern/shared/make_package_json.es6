...
const make_package_json = (
    src_path, metadata, gen_path, deps, dev_deps, cb) => {
    const map_to_str = (i) => {
        const ret = r.map(
            pair => '"' + pair[0] + '": "' + pair[1] + '"',
            r.toPairs(i));
    ...
    fs.readFile(
        src_path + '/lambda_pattern/shared/default_package_values.yaml',
        'utf8',
        cont(err, def_vals_yaml));
    const def_vals = yaml.safeLoad(def_vals_yaml);
    fs.readFile(
        src_path + '/lambda_pattern/shared/package_template.json',
        'utf8',
        cont(err, template));
