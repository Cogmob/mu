...
gulp.task('do_include', function () {
    return gulp.src('src/tools/include_test.js').pipe(webpack({ target: 'node', output: { filename: 'built_tools.js' } })).pipe(gulp.dest('src/tools'));
});

gulp.task('build_dev', sequence('copy_src', 'es6', 'main_file', 'copy_gulpfile_1', 'copy_gulpfile_2', 'do_include'));
